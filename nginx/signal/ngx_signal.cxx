...
typedef struct
{
    ...
    // 信号处理函数,这个函数由我们自己来提供，但是它的参数和返回值是固定的【操作系统就这样要求】,写的时候就先这么写，也不用思考这么多；
    void (*handler)(int signo, siginfo_t *siginfo, void *ucontext); // 函数指针,   siginfo_t:系统定义的结构,该结构包含关于信号的更多信息，如发送信号的进程或用户 ID、额外的错误信息等。
} ngx_signal_t;
...
ngx_signal_t signals[] = {
    ...
    {SIGINT, "SIGINT", ngx_signal_handler},   // 标识2,程序终止信号，通常由 Ctrl+C 发出。
    {SIGTERM, "SIGTERM", ngx_signal_handler}, // 标识15,请求终止信号，通常用于请求程序优雅地关闭。
    {SIGCHLD, "SIGCHLD", ngx_signal_handler}, // 子进程退出时，父进程会收到这个信号--标识17
    {SIGQUIT, "SIGQUIT", ngx_signal_handler}, // 标识3,和 SIGINT 类似，但由 Ctrl+\ 触发，并且通常会导致程序退出并生成核心转储。
...
// 初始化信号的函数，用于注册信号处理程序
// 返回值：0成功  ，-1失败
// 设置进程中需要捕捉的信号和相应的处理程序。
int ngx_init_signals()
{
    ngx_signal_t *sig;   // 指向自定义结构数组的指针
    struct sigaction sa; // 用于定义信号的处理方式。sigaction：系统定义的跟信号有关的一个结构，我们后续调用系统的sigaction()函数时要用到这个同名的结构

    for (sig = signals; sig->signo != 0; sig++) // 将signo ==0作为一个标记，因为信号的编号都不为0；
    {
        // 我们注意，现在要把一堆信息往 变量sa对应的结构里弄 ......
        memset(&sa, 0, sizeof(struct sigaction)); // 清零 sa 结构体。
        ...
        sigemptyset(&sa.sa_mask); // 初始化信号掩码，表示在处理此信号时不阻塞任何其他信号。比如咱们处理某个信号比如SIGUSR1信号时不希望收到SIGUSR2信号，那咱们就可以用诸如sigaddset(&sa.sa_mask,SIGUSR2);这样的语句针对信号为SIGUSR1时做处理，这个sigaddset三章五节讲过；
...
// 多用途的信号处理器，被设计用于处理由 sigaction 设置的多种信号。这个处理器可以灵活地在不同的进程类型中（如 master 和 worker）执行不同的操作，针对不同的信号进行响应。
...
// 这个处理是通过轮询所有子进程的状态来实现的，使用 waitpid 系统调用来检查子进程是否已经结束，并获取其终止状态。
static void ngx_process_get_status(void)
{
    pid_t pid;
    int status;
    int err;
    int one = 0; // 抄自官方nginx，用作标记，确保某些日志只记录一次。

    // 当你杀死一个子进程时，父进程会收到这个SIGCHLD信号。
    for (;;)
    {
        // waitpid，有人也用wait,掌握和使用waitpid即可；这个waitpid说白了获取子进程的终止状态，这样，子进程就不会成为僵尸进程了；
        ...
        if (pid == -1) // 这表示这个waitpid调用有错误，有错误也理解返回出去，我们管不了这么多
        {
            ...
            if (err == ECHILD && one) // 没有子进程,标记至少一次 waitpid 调用成功过.
            {
                return;
            }
        ...
        if (WTERMSIG(status)) // 获取使子进程终止的信号编号,WTERMSIG 提取子进程因接收到的信号而终止的信号编号。如果此值为非零，表示子进程是因为接收到一个信号而异常终止的。
        {
