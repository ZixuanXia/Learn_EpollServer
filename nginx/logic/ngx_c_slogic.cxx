...
// 这个宏计算 statusHandler 数组中的元素总数，用于边界检查，确保在调用函数指针前，指针指向的是有效的成员函数。
...
void CLogicSocket::threadRecvProcFunc(char *pMsgBuf)
{
    ...
    if (m_iLenPkgHeader == pkglen) // 如果包长度等于数据包头的长度，意味着没有包体，只有头部。
    {
        // 没有包体，只有包头
...
// 设计用于检测客户端是否在指定的时间内发送了心跳包，如果客户端没有按时发送心跳包，服务器可以选择主动断开连接。
void CLogicSocket::procPingTimeOutChecking(LPSTRUC_MSG_HEADER tmpmsg, time_t cur_time)
{
    ...
    if (tmpmsg->iCurrsequence == tmpmsg->pConn->iCurrsequence) // 此连接没断
    {
        ...
        if (/*m_ifkickTimeCount == 1 && */ m_ifTimeOutKick == 1) // 能调用到本函数第一个条件肯定成立，所以第一个条件加不加无所谓，主要是第二个条件,它是一个标志位，用于控制是否需要因为超时而踢出用户。
        {
...
// 发送没有包体的数据包给客户端
// 主要功能是将一个不带包体的数据包发送给客户端，通常用于简单的确认或响应包。
...
//----------------------------------------------------------------------------------------------------------
// 处理各种业务逻辑
// 处理客户端的注册请求，验证数据包的合法性，并返回一个响应给客户端。
bool CLogicSocket::_HandleRegister(lpngx_connection_t pConn, LPSTRUC_MSG_HEADER pMsgHeader, char *pPkgBody, unsigned short iBodyLength)
{
    ...
    p_RecvInfo->username[sizeof(p_RecvInfo->username) - 1] = 0; // = 0 是将最后一个字符的位置设置为 '\0'，即 C 字符串的终止符。C 字符串的标准结尾是一个空字符，这样确保了字符串在操作时不会超出预期范围，避免内存访问越界。
...
// 实现了服务器接收到客户端的登录请求后，对包体进行合法性验证和CRC校验，并返回登录响应的过程。
...
// 实现了处理客户端发送的心跳包请求（Ping）的功能。它通过检查包体长度，更新连接的心跳时间，并回复一个只有包头的心跳响应包给客户端。
