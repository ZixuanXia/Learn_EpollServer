...
void CSocekt::zdClosesocketProc(lpngx_connection_t p_Conn) // 处理连接关闭的过程，包括从定时器队列中删除连接、关闭socket描述符，并且将连接放入回收队列以供后续重用。
{
...
bool CSocekt::TestFlood(lpngx_connection_t pConn)
{
    ...
    if ((iCurrTime - pConn->FloodkickLastTime) < m_floodTimeInterval) // 两次收到包的时间 < 100毫秒,比较当前时间 iCurrTime 和上一次记录的时间 pConn->FloodkickLastTime。如果两次时间的间隔小于设定的阈值 m_floodTimeInterval（例如100毫秒），则认为接收数据包的行为可能过于频繁。
    {
        // 发包太频繁记录
...
// 对epoll事件的具体操作
// 返回值：成功返回1，失败返回-1；
// 封装了 epoll_ctl 系统调用的方法，用于操作 Linux 下的 epoll 事件轮询机制。此函数可以添加、修改或删除 epoll 实例中的事件，并管理与每个 socket 描述符（fd）相关联的事件。
int CSocekt::ngx_epoll_oper_event(
    ...
    uint32_t flag,           // 事件类型标志，如 EPOLLIN、EPOLLOUT 等，指定感兴趣的事件或状态。
    int bcaction,            // 补充动作，用于补充flag标记的不足  :  0：增加   1：去掉 2：完全覆盖 ,eventtype是EPOLL_CTL_MOD时这个参数就有用
    lpngx_connection_t pConn // pConn：一个指针【其实是一个连接】，指向连接对象的指针，其中包含 socket 相关的数据。EPOLL_CTL_ADD时增加到红黑树中去，将来epoll_wait时能取出来用
)
{
    struct epoll_event ev; // epoll_event 结构体主要用在 epoll_ctl 函数中注册新的事件监听，以及在 epoll_wait 函数中从 epoll 实例获取已经就绪的事件。
...
// 这个函数调用 epoll_wait 来等待和接收事件，并据此执行相应的处理。这是事件驱动网络编程中非常关键的一个环节，用于异步处理网络I/O事件，提高服务器效率。
int CSocekt::ngx_epoll_process_events(int timer) // timer: 指定 epoll_wait 的超时时间（毫秒）。如果设置为 -1，则会无限阻塞直到事件发生；如果设置为 0，则立即返回，无论事件是否发生。
{
    ...
    for (int i = 0; i < events; ++i) // 遍历本次epoll_wait返回的所有事件，注意events才是返回的实际事件数量
    {
        ...
        if (revents & EPOLLIN) // 如果事件类型包含 EPOLLIN，表明有数据可读，调用连接对象的读处理函数 rhandler。
        {
...
//--------------------------------------------------------------------
// 处理发送消息队列的线程,该线程负责从消息队列中取出消息并发送给相应的客户端。如果发送过程中遇到任何问题，如发送缓冲区已满，它将相应地处理这些情况。
void *CSocekt::ServerSendQueueThread(void *threadData)
{
    ...
    lpngx_connection_t p_Conn;
    unsigned short itmp; // 临时存储转换后的网络数据包长度或其他短整型数据。
    ssize_t sendsize;    // 存储 sendproc 函数的返回值，即发送数据的大小，用于处理部分发送和错误管理。
    ...
    while (g_stopEvent == 0) // 不退出
    {
        ...
        if (pSocketObj->m_iSendMsgQueueCount > 0) // 原子的
        {
            ...
            while (pos != posend)
            {
                ...
                pPkgHeader = (LPCOMM_PKG_HEADER)(pMsgBuf + pSocketObj->m_iLenMsgHeader); // 通过在 pMsgBuf 上加上消息头的长度 m_iLenMsgHeader,来定位到包头的起始位置。包头后面紧跟着的是包体。
                p_Conn = pMsgHeader->pConn;

                if (p_Conn->iCurrsequence != pMsgHeader->iCurrsequence) // 连接对象 p_Conn 中保存的当前序列号与消息头中的序列号不匹配，表明这条消息可能是过期或已不相关的消息，需要从队列中移除，避免发送无效或错误的数据给客户端。
                {
