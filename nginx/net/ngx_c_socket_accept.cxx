...
void CSocekt::ngx_event_accept(lpngx_connection_t oldc)
{
    ...
    socklen_t socklen;          // 用来指定或接收上述 sockaddr 结构的长度。
    int err;
    int level;
    int s;                      // 通常用于存储新接受的socket连接的文件描述符。
    ...
    do // 用do，跳到while后边去方便
    {
        ...
        if (s == -1) // 这表明 accept() 或 accept4() 调用失败。这种情况下，错误信息会存储在全局变量 errno 中。
        {
            err = errno;
            ...
            if (err == EAGAIN) // accept()没准备好，这个EAGAIN错误EWOULDBLOCK是一样的
            {
                ...
                return; // EAGAIN 或 EWOULDBLOCK 错误表示没有更多的连接可以接受，通常发生在非阻塞套接字上。此时，函数直接返回，不再进行更多的操作。
            }
            level = NGX_LOG_ALERT;
            if (err == ECONNABORTED) // ECONNRESET错误则发生在对方意外关闭套接字后【您的主机中的软件放弃了一个已建立的连接--由于超时或者其它失败而中止接连(用户插拔网线就可能有这个错误出现)】
            {
                level = NGX_LOG_ERR; // 如果客户端在建立连接后突然关闭了套接字（可能由于网络问题或客户端崩溃），则降低日志级别到 NGX_LOG_ERR，因为这虽然是错误，但比起其他致命错误来说较为常见。
            }
            ...
            if (use_accept4 && err == ENOSYS) // accept4()函数没实现，坑爹？   ENOSYS:无效的系统调用号
            {
        ...
        if (m_onlineUserCount >= m_worker_connections) // 当前用户数 >= epoll连接的最大数，用户连接数过多，要关闭该用户socket，因为现在也没分配连接，所以直接关闭即可
        {
