...
// 函数的作用是将新的连接 pConn 添加到一个计时队列中，这通常用于处理超时逻辑，如用户连接后在指定时间内没有进行期望的操作则自动断开连接。
void CSocekt::AddToTimerQueue(lpngx_connection_t pConn)
{
	...
	// 分配一块内存用于存储消息头，将连接对象和当前序列号保存到消息头中。这里的序列号用于标识连接的当前状态，防止处理过时的连接事件。
	...
	// 将上述设置的未来时间点和消息头作为一对插入到计时队列 m_timerQueuemap 中。std::multimap 自动根据时间（键）进行排序。
...
// 从计时队列中移除并返回最早的计时器事件。从m_timeQueuemap移除最早的时间，并把最早这个时间所在的项的值所对应的指针 返回，调用者负责互斥，所以本函数不用互斥，
