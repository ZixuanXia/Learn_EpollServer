...
// 参数text：文本写到这里
// 参数len：文本的宽度在这里记录
// 这个函数的作用是将一个 sockaddr 结构体中的IP地址（以及可选的端口号）转换成一个人类可读的字符串格式。这通常用于日志记录、显示连接的客户端地址等。
size_t CSocekt::ngx_sock_ntop(struct sockaddr *sa, int port, u_char *text, size_t len)
{
    struct sockaddr_in *sin; // sin 用于指向转换类型后的 IPv4 地址结构 sockaddr_in
    u_char *p;               // p 用作辅助指针，主要用于存放 ngx_snprintf 的返回值，即新的可写地址。

    switch (sa->sa_family)
    {
    case AF_INET: // 表示IPv4网络地址
