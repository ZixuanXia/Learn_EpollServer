...
void CSocekt::ngx_read_request_handler(lpngx_connection_t pConn)
{
    ...
    if (pConn->curStat == _PKG_HD_INIT) // 连接建立起来时肯定是这个状态，因为在ngx_get_connection()中已经把curStat成员赋值成_PKG_HD_INIT了
    {
        ...
        else
        {
            ...
            pConn->precvbuf = pConn->precvbuf + reco; // 注意收后续包的内存往后走,调整接收缓冲区指针
...
// 通过调用系统的 recv() 函数，它试图从指定的连接 pConn 中读取数据到 buff 缓冲区，直到填满或达到 buflen 指定的最大字节数。函数的返回值和行为提供了关于接收操作状态的重要信息。
...
void CSocekt::ngx_wait_request_handler_proc_p1(lpngx_connection_t pConn, bool &isflood)
{
    ...
    e_pkgLen = ntohs(pPkgHeader->pkgLen); // 注意这里网络序转本机序，所有传输到网络上的2字节数据，都要用htons()转成网络序，所有从网络上收到的2字节数据，都要用ntohs()转成本机序
                                          ...
                                          // 使用 ntohs() 函数将包头中的包长度字段 pkgLen 从网络字节序转换为主机字节序，这是因为网络传输中数字通常以大端字节序传输，而主机的字节序可能不同。
    // 恶意包或者错误包的判断
...
// 注意参数isflood是个引用,函数处理从网络接收到的数据包的包头，并决定如何进一步处理包体。
...
// 是处理写事件的函数，用于向网络发送数据。此函数的主要职责是发送数据，并在发送完成或遇到问题时适当处理。
void CSocekt::ngx_write_request_handler(lpngx_connection_t pConn)
{
    ...
    if (sendsize > 0 && sendsize == pConn->isendlen) // 成功发送完毕，做个通知是可以的；如果 sendsize 等于 isendlen，说明全部数据成功发送。此时可能需要修改epoll事件，移除对可写事件的监听，因为当前没有更多数据需要发送。
    {
